...
import copy
import json
from fontParts.fontshell import RPoint, RGlyph
...
class FFont:
    ...
    def addDiff(self,b,c):
        newFont = FFont(self)
        for key,g in newFont.glyphs.items():
            gB = b.getGlyph(key)
            gC = c.getGlyph(key)
            try:
...
class Master:
    ...
    def openFont(self, path, overlayPath=None):
        self.font = OpenFont(path, showInterface=False)
        for g in self.font:
          size = len(g)
        ...
        if overlayPath != None:
            overlayFont = OpenFont(overlayPath, showInterface=False)
            font = self.font
...
class Mix:
    ...
    def generateFFont(self):
        ...
        for key,g in ffont.glyphs.items():
...
def interpolateKerns(kA, kB, v):
    kerns = {}
    for pair, val in list(kA.items()):
       kerns[pair] = interpolate(val, kB.get(pair, 0), v.x)
    for pair, val in list(kB.items()):
